...
- **M6 (in progress):** the code index ([11](11-index-and-bootstrap.md)). `studio-index` parses GDScript, C# and C++ with tree-sitter into a separate `studio-index.db`, gated on blake3 so an unchanged file is never reparsed, and `symbol_lookup` now answers out of it with a signature, doc and one-hop neighbourhood instead of returning nothing. `studiod studio` bootstraps the index at startup and rescans after every command, emitting `index_updated` when something actually moved. Godot `.tscn`/`.tres` files are parsed into `assets` and `scene_nodes`, so a slice also names the scene node a script is mounted on. Unreal C++ needs its reflection macros blanked before tree-sitter can parse it at all, which measurement caught before the extractor was written. **Unity and UE5 asset extraction are not built; the filesystem watcher was measured and declined** ([11](11-index-and-bootstrap.md)).
