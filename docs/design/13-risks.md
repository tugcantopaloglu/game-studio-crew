...
> **Status:** v0.3. R1 and R2 closed by M1 probes, R0 added after it materialized, R11 added for the unprobed engine profiles, R12-R15 added when the floor and the store were measured.
...
| R14 | **`events_since(run, 0)` on every snapshot, resume and websocket connect.** The store's SQL is bounded by `PRIMARY KEY (run, seq)`, but three call sites in `crates/studio-server/src/lib.rs` read the whole run and filter in Rust. Measured over HTTP against a 50000-event run: **139.9ms to answer "nothing new" with 64 bytes**, and a websocket reconnect wanting 100 events gets its first frame at 139.4ms. It grows with the run for the life of the run. | [03](03-state-store.md), [05](05-event-protocol.md) | **Half open.** The store now offers `head_seq(run)` (0.051ms) and `events_between(run, since, limit)` (0.181ms for a 100-event slice of a 50000-event run), and holds a small pool of read connections with cached statements instead of opening SQLite per call. The three call sites still read from zero. Until they use the bounded API the ceiling is unrealised, and a long-lived run makes every reconnect more expensive than the last. |
