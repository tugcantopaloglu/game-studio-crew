...
**The `notify` watcher below is deliberately not built.** The argument for it was that a scan is O(files hashed) rather than O(edit), so it should not scale. Measured on a synthetic 4001-file Godot project (release build): a cold index costs **2.50s once**, and every subsequent refresh costs **0.24s** whether one file changed or none — roughly 60µs per file, so even a 40k-file project lands near 2.4s. That figure was measured on small text files and extrapolates on file *count*, not bytes; a real project's art directory is bytes, which is what the mtime/size pre-gate below exists to keep out of the hot path. Each command spawns `claude` workers that run for seconds to minutes, so the refresh is under one percent of the command it hangs off.
...
The index is instead kept current by the scans described above, gated on content hashes:

- A scan **blake3 re-hashes** each candidate file. If the hash is unchanged (touch, editor rewrite with identical bytes), **nothing is re-indexed**: the hash gate prevents redundant tree-sitter parses. If changed, only that file's symbols/refs/assets are re-extracted.
- **Binary assets are gated on mtime and size before they are read at all.** Hashing requires reading every byte, and an art directory is most of a real project by volume, so a binary file whose mtime and size both match the indexed row is skipped without opening it. Source files are always read and hashed, because a same-second edit that preserves length is plausible in code and must not be missed.

The two behaviors below are **not built**, and are recorded here as the shape they would take:

- **UE registry dumps would be debounced.** Because dumping the asset registry is an expensive editor commandlet, `.umap`/`.uasset` changes should not trigger a dump per event; they would set a dirty flag and a debounce timer (default a few seconds of quiescence) coalescing a burst of binary changes into one registry dump. This keeps a designer saving maps repeatedly from pinning the editor on back-to-back dumps.
