...
5. **Pad past the model's minimum cacheable prefix or it silently never caches.** Minimums (documented, **not** probed): **Opus 5 = 512 tokens, Fable 5 = 512, Sonnet 5 = 1024, Haiku 4.5 = 4096**. Below the floor `cache_creation` stays 0 with no error, which is why this rule is worth stating twice. The builder pads a short charter to the floor **plus a quarter**, with a stable, meaningful convention block (never whitespace filler, which invites accidental edits). **The margin is headroom, not a correction for the estimator — the estimator errs in the safe direction, twice over.** `estimate_tokens` counts only the charter text, at an assumed 3.6 characters a token. Two things push the real number above it, both measured. Dense charter prose tokenizes nearer **2.52** characters a token (a 2333-character charter came back as 926 tokens, 1.43× the estimate), because the words are short, the punctuation heavy, and the numbered padding lines spend a token per digit. And the cached prefix is the charter **plus the tool schemas**, which the estimator never sees at all: with `Read,Grep,Glob` attached, a 651-token estimate cached as **3666** real tokens, 5.6×. Treat an estimate as a conservative **lower** bound: it clears the floor before the margin is applied. The trap is the inverse reading, which makes the margin look like slack worth trimming and an estimate look like a ceiling worth sizing against. Two traps live here. The floors are **not monotonic across generations** (Opus 4.6 = 4096, Opus 4.7 = 2048, Opus 4.8 = 1024, Opus 5 = 512), so a figure borrowed from the neighbouring version reads plausible and is wrong; and this file previously named Opus 4.8's 4096-token floor, which is two generations stale. M1 measurements are consistent with the rule (a 184-token prefix produced `cache_creation: 0`; an 8867-token prefix cached), but no threshold here has been isolated by probe — confirming one costs a spawn per model, reading `cache_creation` off the result.
...
**Measured, and re-measured after the padding changed.** The figures below are M1 probe measurements at $5.00/MTok input, cache read 0.1× base, cache write 2.0× base at the 1-hour TTL. The L3 column is still projected, because a representative task brief does not exist until real workflows run; the ledger ([03](03-state-store.md)) settles it.

Per Tier-2 (Opus) invocation, `--tools "Read,Grep,Glob"`, cold then warm on the same prefix:

| | at 4096-token padding | at floor-plus-a-quarter padding |
|---|---|---|
| charter, estimated | 4096 | **651** (197 of it padding) |
| prefix actually cached | 8867 | **3666** |
| cold | $0.0888 | **$0.0374** |
| warm | $0.0051 | **$0.0025** |
| warm-to-cold | 17.4× | **14.8×** |

**The charter is the small part of what gets cached.** 651 estimated charter tokens produced a 3666-token cached prefix, because the prefix is the charter **plus the tool schemas**, and `estimate_tokens` counts only the charter. That is the dominant reason an estimate reads low — a 5.6× gap here against 1.43× for the same charter with an empty tool list, where only the characters-per-token effect is left. It is also the measurement that settles which lever matters: cutting the padding by 3400 estimated tokens moved the cached prefix by 5201, while the tool allowlist alone accounts for the rest and for the whole 22572 → 8867 reduction below.

The **2.0× write premium survives the change**: 3666 × ($5.00 × 2.0)/1e6 = $0.0367 against $0.0374 measured with 4 output tokens included. ✓ So does the shape of the argument — a cold spawn is dear, a warm one is nearly free, and the way to win is fewer distinct prefixes ([ADR 0002](adr/0002-thirteen-roles.md)).

The 22572-token undefended figure below is a property of the **default tool set**, not of padding, so it is unaffected in kind — but it has not been re-measured since, and the padding reduction applies to it too. Treat it as an upper bound rather than a current reading.
