...
**No longer an estimate, but no longer current either.** The prefix figures below are M1 probe measurements taken against a charter padded to **4096 tokens**, at $5.00/MTok input, cache read 0.1× base, cache write 2.0× base at the 1-hour TTL. They were real and the ratios they establish still hold. What has changed is the input: the padding target is now the model's floor plus a quarter, so an Opus charter freezes to roughly **655** tokens rather than 8867. Every absolute figure in this table is therefore historical — the 17.4× warm-to-cold ratio and the 2.0× write premium are properties of the cache and survive, but the dollar amounts describe a prefix the studio no longer builds. Re-measure before quoting them; the same M1 probe path produces the new numbers. The L3 column was always projected, because a representative task brief does not exist until real workflows run; the ledger ([03](03-state-store.md)) settles it.
