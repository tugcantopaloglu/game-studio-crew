...
The doctor reports **both** facts, and never lets the first pass for the second. Each installed CLI is tagged either *the studio spawns workers through this* or *installed, but the studio cannot drive it*, and a refusal is never left as a bare boolean: the row underneath carries `why:` and the provider's own reason, cut to the clause that names the problem so three long blockers do not become a wall of text. An install where nothing is drivable prints the reasons in full and says to install Claude Code. This exists so nobody installs, sees a tick next to "gemini found", and discovers at the first task that no worker can start.

```
  claude    2.1.220 (Claude Code)   the studio spawns workers through this
  codex     codex-cli 0.145.0       installed, but the studio cannot drive it
            why: codex takes its instructions from AGENTS.md and has no flag that replaces the system prompt
  kimi      absent
```

The verdict is **not this module's opinion**. `drivable` is `studio_core::Provider::can_serve(RoleNeeds::default())` and the reason text is the first line of `Provider::blockers`, so the doctor, the settings panel and the supervisor cannot disagree. A test asserts row by row that the report agrees with `Provider::ALL`, and the tests assert *relationships* rather than sentences — that a refusal carries a non-empty reason, and that what the doctor prints is a prefix of the provider's own words. An earlier version of them quoted the wording instead and went red the day `Provider::Codex` was added, while the doctor itself had followed the table correctly; the tests, not the code, were what needed fixing. **If the provider table gains a CLI or a capability, this report follows it automatically; the only thing that must be kept in step by hand is the `CODING_CLIS` probe list, which is what decides whether a CLI is looked for at all.**
