...
Measured effect: a warm invocation's prefix costs **$0.0051 against $0.0888 cold and $0.2258 undefended, a 17.4× warm-to-cold reduction**, across separate subprocesses. These are M1 probe measurements, not estimates — taken against a charter padded to 4096 tokens. The padding target has since dropped to each model's real minimum cacheable prefix plus a quarter, so an Opus charter is now about 655 tokens rather than 8867: the ratios above are properties of the cache and still hold, but the absolute figures describe a prefix the studio no longer builds and want re-measuring. See [`02-context-engine.md`](docs/design/02-context-engine.md) and [`probes/`](probes/README.md).
